  ...
  def add(context: SequenceContext) =
  {
    ...
    arpeggio match
    {
      case Some(Arpeggio(beat, sequence)) =>
      {
        ...
        sequence.take(patternSequenceCount-1).zipWithIndex.map{
          case (index: Int, i: Int) => 
            ...
            if (index <= 0 || index > notes.length)
              Rest.add(context.copy(beat=beat, position=context.position+Timing(beat * i, 1)))
            else
              //  Otherwise play the indexed note from those in the chord (indexed low-to-high, 1-based)
              notes(index-1).add(context.copy(beat=beat, position=context.position+Timing(beat * i, 1)))
        }
        ...
        if (index <= 0 || index > notes.length)
          Rest.add(context.copy(beat=new Beat(remainingTiming), position=context.position+Timing(beat * lastInPattern, 1)))
        else
          notes(index-1).add(context.copy(beat=new Beat(remainingTiming), position=context.position+Timing(beat * lastInPattern, 1)))
